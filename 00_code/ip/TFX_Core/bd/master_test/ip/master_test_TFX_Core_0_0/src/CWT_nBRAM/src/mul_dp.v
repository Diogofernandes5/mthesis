...
(
    ...
    input wire bram_fft_en_i,
    input wire bram_fft_we_i,
    input wire [$clog2(N)-1:0] bram_fft_addr_i,
...
wire [31:0] fft_re_bram;
wire [31:0] fft_im_bram;
...
wire [15:0] brom_daughter;

/* Output of the mul results BRAM */
wire [31:0] mul_re_bram;
wire [31:0] mul_im_bram;

assign rst = ~rstn;

/* Daughter wavelet coeficients to multiply by the fft results */ 
brom_daughter_ip daughter_wave (
...
bram_N_point bram_fft_re (
    .clka(~clk),    // input wire clka
    .rsta(rst),
    .ena(bram_fft_en_i),      // input wire ena
    .wea(bram_fft_we_i),      // input wire [0 : 0] wea
    .addra(bram_fft_addr_i),  // input wire [9 : 0] addra
    .dina(x_re_i),    // input wire [31 : 0] dina
    .douta(fft_re_bram)  // output wire [31 : 0] douta
);

bram_N_point bram_fft_im (
    .clka(~clk),    // input wire clka
    .rsta(rst),
    .ena(bram_fft_en_i),      // input wire ena
    .wea(bram_fft_we_i),      // input wire [0 : 0] wea
    ...
    .douta(fft_im_bram)  // output wire [31 : 0] douta
);
...
mux2_0 mult_x_re_mux ( // if number is negative, invert the number to make the multiplication
    ...
    .s(fft_re_bram[31] ^ brom_daughter[15]),
    .y(x_re_m)
);
...
mux2_0 mult_x_im_mux ( // if number is negative, invert the number to make the multiplication
    ...
    .s(fft_im_bram[31] ^ brom_daughter[15]),
    .y(x_im_m)
);

bram_N_point bram_mul_re (
    .clka(~clk),    // input wire clka
    .rsta(rst),
    ...
    .douta(mul_re_bram)  // output wire [31 : 0] douta
);

bram_N_point bram_mul_im (
    .clka(~clk),    // input wire clka
    .rsta(rst),
    .ena(bram_mul_en_i),      // input wire ena
    .wea(bram_mul_we_i),      // input wire [0 : 0] wea
    ...
    .douta(mul_im_bram)  // output wire [31 : 0] douta
);

assign X_re_o = mul_re_bram;
assign X_im_o = mul_im_bram;
