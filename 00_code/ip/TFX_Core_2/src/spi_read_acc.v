`define ADXL313_DATA_X0 32
`define ADXL313_DATA_Y0 34
`define ADXL313_DATA_Z0 36
...
reg bram_we;
reg [7:0] bram_addr;
reg [ACC_RESOLUTION-1:0] bram_din;

reg [10:0] data_counter;
reg [2:0] state;
wire [ACC_RESOLUTION-1:0] axis_dt_pnt;
reg int_flag;
...
always @(posedge clk or negedge rstn) begin
	if (!rstn) begin
		...
		bram_latency_shift <= {bram_latency_shift[0], state == S_SEND};
...
assign data_o = data_valid_o ? bram_data: 0;
...
acc_data_bram data_bram (
	...
	.douta(bram_data)  // output wire [12 : 0] douta
);
