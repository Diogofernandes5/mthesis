...
uint8_t fft(complex *y_io, uint32_t N_i, complex *aux_i)
{
    ...
    complex *vo = aux_i;                    /* even array points to first position */ 
    complex *ve = aux_i + half_N_i;         /* odd  array points to middle position */
    ...
    for(int m = 0; m < (half_N_i + 1); m++) 
    {
        /* Twiddle factor:
            w = exp(-2i*PI*m/N_i):
            	Re(e^-ix) = cos(x)
            	Im(e^-ix) = -sen(x) */
        w.Re =  cos((2 * M_PI * m) / (double)N_i);
        w.Im = -sin((2 * M_PI * m) / (double)N_i);
