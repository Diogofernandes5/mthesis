module control_unit #(
    ...
    output reg [$clog2(N)-1:0] bram_addr_x0_o,
    output reg [$clog2(N)-1:0] bram_addr_x1_o,
...
reg [$clog2(N)-1:0] group_start;              // variable that defines the starting address of a group
//reg [9:0] group_counter;            // variable that deermines the current group of butterflies
reg [$clog2(N/2)-1:0] local_index;              // variable that defines the index inside of the group we are currently
wire [$clog2(N)-1:0] bram_addr_x0;           // summation result auxiliary used for bram_addr when needed
wire [$clog2(N)-1:0] bram_addr_x1;           // summation result auxiliary used for bram_addr when needed
    ...
    if(~rstn) begin
        ...
        case(state)
            ...
            S_WRITE_BACK: begin
                ...
                twiddle_addr_ad <= twiddle_addr_o + staged_half_N;
            end
...
always @(posedge clk or negedge rstn) begin
    if(~rstn) begin
        ...
        // group_start = group_counter * 2^(s+1)
...
data_counter_mux2 your_instance_name (
...
adder_subtracter32bit bram_addr_x1_adder (
...
div_10bit staged_half_N_div (
  ...
  .b({0,group_size}),  // input wire [9 : 0] divisor
