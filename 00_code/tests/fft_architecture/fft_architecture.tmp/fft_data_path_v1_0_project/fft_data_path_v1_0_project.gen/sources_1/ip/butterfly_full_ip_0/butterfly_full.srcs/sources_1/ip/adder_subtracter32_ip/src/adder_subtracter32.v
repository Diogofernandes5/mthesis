...
module adder_subtracter32(
    ...
    assign v = c_out[31] ^ c_out[30]; // overflow
    
    assign positive_v = ~x[31] && ((!y[31] && !c_in) || (!y[31] && !c_in)) && v; // positive overflow
    assign negative_v = x[31] && (y[31] ^ c_in) && v;  // negative overflow
    
    always @* begin
        if (negative_v) begin // negative
            r <= 32'h80000000; // saturate negative
        end 
        else if (positive_v)  begin
            r <= 32'h7FFFFFFF; // saturate positive
        end
