...
// void i2s_read(i2s_comm_desc *hi2s_desc, uint32_t *_buff)
// {
  // volatile HAL_StatusTypeDef hal_status = HAL_I2S_Receive(&hi2s_desc->hi2s, data_in, 2, 100);
  ...
  // uint16_t data_in[2];
  // HAL_StatusTypeDef hal_status = HAL_I2S_Receive(&hi2s_desc->hi2s, data_in, 2, 100);

  // if(hal_status == HAL_OK) 
  // {
  //   int32_t data_full = (int32_t) data_in[0] << 16 | data_in[1];
  //   int16_t data_short = (int16_t) data_in[0];
  // }
// }
