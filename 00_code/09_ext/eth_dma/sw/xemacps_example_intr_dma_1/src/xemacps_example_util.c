...
#define PHY_ID_REALTEK    0x001C  // From PHY register 2
#define PHY_REG_BMCR      0x00
#define BMCR_LOOPBACK     0x4000
#define BMCR_AUTONEGT     0x1000
#define BMCR_SPEED_1000   0x0040
#define BMCR_SPEED_100    0x2000
#define BMCR_SPEED_10     0x0000
#define BMCR_FULLDUPLEX   0x0100
#define BMCR_RESET        0x8000

#define PHY_R1_AN_COMPLETE 	0x0020
#define PHY_R1_LINK_UP		0x0004

LONG EmacPsUtilRealtekPhyLoopback(XEmacPs *EmacPsInstancePtr, u32 Speed, u32 PhyAddr)
{
    u16 ControlReg = 0;
    LONG status = 0;
    u16 PhyReg = 0;

    // Set loopback, speed, and full duplex
    ControlReg |= BMCR_LOOPBACK | BMCR_FULLDUPLEX | BMCR_AUTONEGT;

    // disable loopback
    ControlReg &= ~BMCR_LOOPBACK;

    switch (Speed) {
        case 1000:
            ControlReg |= BMCR_SPEED_1000;
            break;
        case 100:
            ControlReg |= BMCR_SPEED_100;
            break;
        case 10:
            ControlReg |= BMCR_SPEED_10;
            break;
        default:
            xil_printf("Unsupported speed %lu for Realtek PHY\r\n", Speed);
            return XST_FAILURE;
    }

    // Write to BMCR to enable loopback
    XEmacPs_PhyWrite(EmacPsInstancePtr, PhyAddr, PHY_REG_BMCR, ControlReg);

    // Optional: wait for loopback to stabilize
    usleep(100); // 100 ms

//    status = XEmacPs_PhyRead(EmacPsInstancePtr, PhyAddr, 1, &PhyReg);
    do {
    	XEmacPs_PhyRead(EmacPsInstancePtr, PhyAddr, 1, &PhyReg);
    	sleep(1);
    } while(!(PhyReg & PHY_R1_AN_COMPLETE));
...
LONG EmacPsUtilEnterLoopback(XEmacPs * EmacPsInstancePtr, u32 Speed)
{
	...
	if (PhyIdentity == PHY_ID_REALTEK) {
		Status = EmacPsUtilRealtekPhyLoopback(EmacPsInstancePtr, Speed, PhyAddr);
	}

	xil_printf("Phy ID = %d; Phy Addr = %d\r\n", PhyIdentity, PhyAddr);
