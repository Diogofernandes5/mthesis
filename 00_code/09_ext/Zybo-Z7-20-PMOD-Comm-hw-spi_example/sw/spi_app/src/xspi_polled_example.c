...
int SpiPolledExample(XSpi *SpiInstancePtr, u16 SpiDeviceId)
{
	...
	Status = XSpi_SetOptions(SpiInstancePtr, XSP_MASTER_OPTION |
					XSP_MANUAL_SSELECT_OPTION | XSP_CLK_ACTIVE_LOW_OPTION | XSP_CLK_PHASE_1_OPTION);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	...
	u8 _address = 0x80 | 0x00;
	//_address |= 0x40;

	ReadBuffer[0] = _address;

	XSpi_Transfer(SpiInstancePtr, ReadBuffer, &ReadBuffer[1], 2);
	...
	for (Count = 0; Count < BUFFER_SIZE; Count++) {
		...
		WriteBuffer[Count] = 0x80 | 0x02 | 0x40;
		ReadBuffer[Count] = 0;
	}
