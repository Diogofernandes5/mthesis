...
#define ACC_INT 			XPAR_INTC_0_ACC_INT_VEC_ID
...
/*
 * The following constants define the offsets within a AccBuffer data
...
#define WRITE_DATA_OFFSET	1  /* Start of data to write to the EEPROM */
#define READ_DATA_OFFSET	4  /* Start of data read from the EEPROM */
...
#define BUFFER_SIZE     16//MAX_DATA + READ_DATA_OFFSET
 ...
 */
typedef u8 AccBuffer[BUFFER_SIZE];
...
void AccRead(XSpi *SpiPtr, u8 Address, int ByteCount, AccBuffer Buffer);

void AccWrite(XSpi *SpiPtr, u8 Address, u8 ByteCount, AccBuffer Buffer);
...
int main(void)
{
	 ...
	 */
	Status = XSpi_SetSlaveSelect(&Spi, SACC_SPI_SELECT);
	if (Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	...
	/**
	 * Read the DEVID_0 to check if device is present 
	 * */
	AccRead(&Spi, ADXL313_PARTID, 1, ReadBuffer);
	BufferPtr = &ReadBuffer[READ_DATA_OFFSET];
	xil_printf("PartID = %x", *BufferPtr);
	...
	// 	AccWrite(&Spi, Page * PAGE_SIZE, PAGE_SIZE,
	...
	//  * complete
	//  */
	// AccRead(&Spi, 0, MAX_DATA, ReadBuffer);
...
void AccRead(XSpi *SpiPtr, u8 Address, int ByteCount, AccBuffer Buffer)
{
	...
	u8 _address = 0x80 | Address;

	/* If Multi-Byte Read: Bit 6 Set */
	if(ByteCount > 1) 
		_address |= 0x40;

	Buffer[COMMAND_OFFSET] = _address;
