...
#define TX_DONE_INTR_ID XPAR_AXI_INTC_0_AXI_MASTER_FULL_0_TX_DONE_INTR_INTR
#else
#define RX_INTR_ID		XPAR_FABRIC_AXIDMA_0_S2MM_INTROUT_VEC_ID
#define TX_INTR_ID		XPAR_FABRIC_AXIDMA_0_MM2S_INTROUT_VEC_ID
#define TX_DONE_INTR_ID XPAR_FABRIC_AXI_MASTER_FULL_0_TX_DONE_INTR_INTR
#endif
...
static int SetupInterruptSystem(INTC * IntcInstancePtr,
			   XAxiDma * AxiDmaPtr, u16 TxIntrId, u16 RxIntrId, u16 TxDoneId);
static void DisableIntrSystem(INTC * IntcInstancePtr,
					u16 TxIntrId, u16 RxIntrId, u16 TxDoneId);
...
int main(void)
{
	...
	Status = SetupInterruptSystem(&Intc, &AxiDma, TX_INTR_ID, RX_INTR_ID, TX_DONE_INTR_ID);
	if (Status != XST_SUCCESS) {
...
//	DisableIntrSystem(&Intc, TX_INTR_ID, RX_INTR_ID, TX_DONE_INTR_ID);
...
{
    // 1. Your custom handling code here
    xil_printf("TX Done interrupt occurred!\r\n");

    // 2. Clear interrupt at peripheral level (if needed)
//     *(volatile uint32_t *)PERIPH_BASE_ADDR = 0x1;

    // 3. Acknowledge interrupt in INTC
//    XIntc_Acknowledge(&InterruptController, XPAR_AXI_INTC_0_AXI_MASTER_FULL_0_TX_DONE_PULSE_INTR);
}
			   ...
			   XAxiDma * AxiDmaPtr, u16 TxIntrId, u16 RxIntrId, u16 TxDoneId)
{
	...
	Status = XIntc_Connect(IntcInstancePtr, TxDoneId,
				   (XInterruptHandler) RxIntrHandler, RxRingPtr);
	if (Status != XST_SUCCESS) {
	...
	XIntc_Enable(IntcInstancePtr, TxDoneId);
	...
	XScuGic_SetPriorityTriggerType(IntcInstancePtr, TxDoneId, 0xA0, 0x3);
	 ...
	 */
	Status = XScuGic_Connect(IntcInstancePtr, TxDoneId,
	...
	XScuGic_Enable(IntcInstancePtr, TxDoneId);
...
//{
//    // 1. Initialize INTC
//    if (XIntc_Initialize(IntcInstancePtr, INTC_DEVICE_ID) != XST_SUCCESS) {
//        xil_printf("INTC init failed\n");
//        return XST_FAILURE;
//    }
//
//    // 2. Clear ANY existing pending interrupts
////    u32 pending = XIntc_GetIsr(IntcInstancePtr);
////    if (pending) {
////        xil_printf("Clearing existing interrupts: 0x%08X\n", pending);
////        for (int i = 0; i < XPAR_INTC_MAX_NUM_INTR_INPUTS; i++) {
////            if (pending & (1 << i)) {
////                XIntc_Acknowledge(IntcInstancePtr, i);
////            }
////        }
////    }
//
//    // 3. Connect handler
//    if (XIntc_Connect(IntcInstancePtr, TxDoneId,
//		       (XInterruptHandler) TxDone_Handler, IntcInstancePtr) != XST_SUCCESS) {
//        xil_printf("Handler connect failed\n");
//        return XST_FAILURE;
//    }
//
//    // 4. Start the controller
//    if (XIntc_Start(IntcInstancePtr, XIN_REAL_MODE) != XST_SUCCESS) {
//        xil_printf("INTC start failed\n");
//        return XST_FAILURE;
//    }
//
//    // 5. Enable this specific interrupt
//    XIntc_Enable(IntcInstancePtr, TxIntrId);
//
//    // 6. Enable processor interrupts
//    Xil_ExceptionInit();
//    Xil_ExceptionRegisterHandler(XIL_EXCEPTION_ID_INT,
					...
					u16 TxIntrId, u16 RxIntrId, u16 TxDoneId)
{
	...
	XScuGic_Disconnect(IntcInstancePtr, TxDoneId);
#endif
}
