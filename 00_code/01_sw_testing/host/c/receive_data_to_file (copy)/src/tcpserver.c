...
void init_tcpserver(int port, int *sd_listen, struct sockaddr_in *sock_addr, int max_client_num)
{
	...
	*sd_listen = socket(PF_INET, SOCK_STREAM, 0);
	...
	memset(&sock_addr, 0, sizeof(sock_addr));	/*set sock_addr as 0000... (sizeof(sock_addr))*/
	sock_addr->sin_family = AF_INET;		/* protocol family*/
	sock_addr->sin_port = port;
	sock_addr->sin_addr.s_addr = INADDR_ANY;                   /* any interface */
