...
f0 = 150;                       % Frequency of first sine wave
f1 = 800;                       % Frequency of second sine wave
...
%s0 = fc/pi;
s0_ind = 4;           % increasing the s0_ind, will make the wavelet start lower in freq                    
s0 = s0_ind*dt;
...
for i=1:1:1024
    tic
    % 
    % f0 = bitand((f0+20),0x7ff);                       % Frequency of first sine wave
    % f0 = double(f0);
    % x = sin(2 * pi * f0 * t); % Generated signal
    f0 = double(800 + (1024-800)*rand(1));
    f1 = double(300 + (500-300)*rand(1));
    x = sin(2 * pi * f0 * t) + 2*sin(2 * pi * f1 * t); % Generated signal
    x = fix(x * 2^fractional_len);
    x = complex(x,zeros(1,N));

    %% Wavelet transform:
    [wt,period,scale,freq, coi, daughter] = wavelet(x, fs, no, vpo, s0, mother, omega, pad, fractional_len);
    % [wt,~,~,~,~,~] = wavelet(x, fs, no, vpo, s0, mother, omega, pad, fractional_len);
