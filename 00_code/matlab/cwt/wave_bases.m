...
function [daughter,fourier_factor,e_folding,dofmin] = ...
	wave_bases(mother,k,scale,param, fs);
...
if (strcmp(mother,'MORLET'))  %-----------------------------------  Morlet
	...
	norm = (pi^(-0.25))*sqrt(n);    % total energy=N   [Eqn(7)]
...
elseif (strcmp(mother,'PAUL'))  %--------------------------------  Paul
    ...
    expnt = -(scale.*k);
	norm = 2^m / sqrt(m*factorial(2*m-1));
...
elseif (strcmp(mother,'DOG'))  %--------------------------------  DOG
	...
	m = param;
	expnt = -(scale.*k).^2 ./ 2.0;
    norm = -((1i^m) / sqrt(gamma(m+0.5))) .* (scale*k).^m;
	daughter = norm.*exp(expnt);
